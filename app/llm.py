...
"""
    ...
    try:
        ...
        if provider == "gemini":
            # NOTE: `google.generativeai` is the retired SDK and is not a
            # dependency of this project. Importing it here raised
            # ModuleNotFoundError on the first line of this branch, so every
            # Gemini support reply fell through to the canned fallback.
            from google import genai
            from google.genai import types
            client = genai.Client(api_key=os.getenv("GEMINI_API_KEY", ""))
            response = await client.aio.models.generate_content(
